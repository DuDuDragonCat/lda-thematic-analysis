...
def make_filename(folder_path, filename_prefix):
    date = datetime.datetime.now().strftime('%Y%m%dT%H%M%S')
    filename = f'{filename_prefix}.xlsx'
    filename_date = f'{filename_prefix}_{date}.xlsx'
...
def run_lda_full(word_lists, topic_nums, seed, burn_in, iteration, thin):
...
def _caojuan2009(models):
    ...
    for k, mdl in models.items():
        ...
        sims = [np.dot(phi[i], phi[j]) / (np.linalg.norm(phi[i]) * np.linalg.norm(phi[j]))
                for i, j in pairs]
...
def _arun2010(models, word_lists):
    ...
    for k, mdl in models.items():
        ...
        result[k] = float(np.sum(cm1 * np.log(cm1 / cm2)) + np.sum(cm2 * np.log(cm2 / cm1)))
    return result
...
def _deveaud2014(models):
    ...
    for k, mdl in models.items():
        ...
        pairs = [(i, j) for i in range(k) for j in range(i + 1, k)]
        jsds = [0.5 * np.sum(phi[i] * np.log(phi[i] / phi[j]))
...
_MARKERS  = {'Griffiths2004': 'o', 'CaoJuan2009': 's',
             'Arun2010': '^', 'Deveaud2014': 'D'}
...
def save_metrics_plot(df_norm, save_path):
    ...
    for panel_label, metrics, ax in [
        ...
        for metric in metrics:
            ax.plot(ks, df_norm[metric].tolist(),
                    marker=_MARKERS[metric], label=metric, linewidth=1.5, markersize=7)
...
def build_metrics_df(models, log_lik_samples, word_lists):
    ...
    rows = [{'k': k, 'Griffiths2004': g[k], 'CaoJuan2009': c[k],
             'Arun2010': a[k], 'Deveaud2014': d[k]} for k in valid_ks]
...
def run_findbest(input_files, output_folder, seed, burn_in, iteration, thin,
                 min_topic, max_topic, per_topic, on_append, on_done, on_error):
    """啟動背景執行緒執行 LDA 指標計算。"""

    def _worker():
        try:
            on_append(f'SEED: {seed}; Burn In: {burn_in}; Iteration: {iteration}; Thin: {thin}')
            on_append(f'Topic Number From: {min_topic}; To: {max_topic}; By: {per_topic}')
            ...
            models, log_lik_samples, _ = run_lda_full(
                word_lists, topic_nums, seed, burn_in, iteration, thin)
            ...
            out_xlsx = make_filename(output_folder, 'LDA_Metrics')
            result_df.to_excel(out_xlsx, index=False)
            on_append(f'Excel 已存至：{out_xlsx}')

            norm_cols = ['k', 'Norm_Griffiths2004', 'Norm_CaoJuan2009',
                         'Norm_Arun2010', 'Norm_Deveaud2014']
            df_plot = result_df[norm_cols].rename(columns={
